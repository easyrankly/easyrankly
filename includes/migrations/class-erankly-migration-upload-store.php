...
/** Stages validated source exports outside the public WordPress tree. */
final class ERankly_Migration_Upload_Store {
	...
	public static function store_http_upload( array $file, string $requested_source = 'auto' ): array {
		...
		return self::stage( $tmp_name, $name, $requested_source, true );
	}
	...
	public static function stage_trusted_file( string $path, string $requested_source = 'auto' ): array {
		return self::stage( $path, basename( $path ), $requested_source, false );
	}
	...
	/**
	 * Copies or moves, detects and validates one source file.
	 *
	 ...
	 * @param string $requested_source auto or a supported adapter slug.
	 * @param bool   $move_upload     Whether move_uploaded_file() is required.
	 * @return array<string,mixed>
	 */
	private static function stage( string $source_path, string $original_name, string $requested_source, bool $move_upload ): array {
		...
		$destination = $directory . '/' . self::FILE_PREFIX . $random . '.' . $extension;
		$stored      = $move_upload
			? move_uploaded_file( $source_path, $destination )
			: copy( $source_path, $destination ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_copy -- Trusted fixtures and HTTP temp files are copied only into non-public managed storage.
