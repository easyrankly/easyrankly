...
final class ERankly_ML_Repository {
	...
	public function link( int $group_id, int $blog_id, string $object_type, int $object_id ): int {
		...
		// Insert the membership row. For a brand-new group we insert with a temporary
		// group_id of 0 and then adopt this row's own AUTO_INCREMENT id as the group
		// id. That allocation is atomic, so two concurrent "create group" requests can
		// never receive the same id the way MAX(group_id)+1 could.
		$sql = $wpdb->prepare(
		...
		if ( $create_group ) {
			...
			$wpdb->query( // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Adopts the new row's unique id as its group id.
				$wpdb->prepare( 'UPDATE %i SET group_id = %d WHERE id = %d', $table, $group_id, $group_id )
			);
		}
