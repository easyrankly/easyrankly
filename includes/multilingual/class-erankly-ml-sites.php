...
final class ERankly_ML_Sites {
	...
	/**
	 * Removes a site from the language map (e.g. when the site is deleted).
	 *
	 * The relation rows are cleaned up separately by the repository; this drops
	 * the leftover language-map entry so a deleted blog does not linger in the
	 * network option.
	 *
	 * @param int $blog_id Blog ID of the removed site.
	 * @return void
	 */
	public static function remove_site( int $blog_id ): void {
		...
		$was_default = ! empty( $all[ $blog_id ]['is_default'] );
		unset( $all[ $blog_id ] );

		// If the removed site held x-default, promote the first remaining enabled
		// site so hreflang output keeps a valid fallback.
		if ( $was_default ) {
