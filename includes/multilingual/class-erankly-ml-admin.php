...
final class ERankly_ML_Admin {
	...
	public function register_hooks(): void {
		...
		// Note: the network_admin_edit_erankly_ml_sites_save endpoint is wired
		// in includes/admin.php, which boots this module on demand and routes to
	...
	public function register_rest_routes(): void {
		...
		register_rest_route(
			...
			array(
				...
				'callback'            => array( $this, 'rest_save_ml_sites' ),
				// Always the network capability: this panel is Network-Admin-only
				// (see render_network_panel()'s own is_multisite() guard and the
				// admin/settings-page.php condition that renders it), matching
				// what save_ml_sites() itself already requires.
				'permission_callback' => static fn() => current_user_can( 'manage_network_options' ),
				'args'                => array(
					'settings' => array(
						'type'     => 'object',
						'required' => true,
	...
	/**
	 * Saves the site-language map autosave payload.
	 *
	...
	public function update_post_translations_rest_field( mixed $value, WP_Post $post ): bool|WP_Error {
		...
		foreach ( $value as $row ) {
			...
			if ( $object_id > 0 ) {
				...
				// Re-pointing an existing relation needs no explicit unlink here:
				// the centralized process_object_links() path runs the capability
				// gate and repo->link() drops any stale slot for this blog. Doing
				// it earlier would detach a relation before that check.
				$raw[ $blog_id ] = array(
					'object_id' => $object_id,
					'action'    => $action,
				);
	...
	private function search_posts( string $query ): array {
		...
		foreach ( $wp_query->posts as $post ) {
			...
			$results[] = array(
				'id'    => $post->ID,
				'title' => $this->decode_admin_label( get_the_title( $post ) ),
	...
	/**
	 * Decodes HTML entities for labels returned to admin text controls.
	 *
	 * WordPress may store smart punctuation in titles as numeric entities
	 * (for example &#8217;). REST/React renders these values as text, so decode
	 * them before they reach the combobox label.
	 *
	 * @param string $label Raw title or term name.
	 * @return string
	 */
	private function decode_admin_label( string $label ): string {
	...
	private function search_terms( string $query ): array {
		...
		foreach ( $terms as $term ) {
			...
			$results[] = array(
				'id'    => $term->term_id,
				'title' => $this->decode_admin_label( $term->name ),
	...
	private function process_object_links( int $source_blog_id, string $object_type, int $source_object_id, array $raw ): void {
		...
		foreach ( $raw as $blog_id => $link_data ) {
			...
			// Broken-access-control guard: a request must never touch a relation
			// on a network site the current user does not belong to. Without this
			// a user could forge a POST that links or unlinks content on any other
			// site of the network, even one they cannot access.
			if ( ! $this->user_can_access_site( $blog_id ) ) {
				continue;
			}

			if ( 'unlink' === $action ) {
				// Only detach a relation that actually belongs to the source
				// object's own group, so a forged request cannot remove the
				// translations of unrelated content (or the source itself).
				if ( $group_id > 0
					&& $blog_id !== $source_blog_id
					&& $this->group_contains_object( $group_id, $blog_id, $object_type, $object_id )
					&& $this->user_can_edit_target( $blog_id, $object_type, $object_id ) ) {
	...
	/**
	 * Whether the current user may manage relations on a given network site.
	 *
	 * Super admins reach every site; everyone else must be a member of the
	 * target blog. Mirrors the membership gate used by the REST search route so
	 * the save path cannot be tricked into touching sites the user cannot see.
	 *
	 * @param int $blog_id Target site ID.
	 * @return bool
	 */
	private function user_can_access_site( int $blog_id ): bool {
		if ( ! is_multisite() ) {
			return true;
		}

		return is_super_admin() || is_user_member_of_blog( get_current_user_id(), $blog_id );
	}

	/**
	 * Whether the current user may edit a specific translation target on its site.
	 *
	 * Stronger than mere site membership: the user must hold edit_post/edit_term
	 * for the target object on its own blog. This prevents a member who cannot
	 * edit a given post or term from linking or unlinking translation relations
	 * that point to it. Switching blogs re-evaluates the user's capabilities for
	 * that site, so the check is accurate across the network.
	 *
	 * @param int    $blog_id     Target site ID.
	 * @param string $object_type 'post' or 'term'.
	 * @param int    $object_id   Target object ID.
	 * @return bool
	 */
	private function user_can_edit_target( int $blog_id, string $object_type, int $object_id ): bool {
		if ( ! $this->user_can_access_site( $blog_id ) ) {
			return false;
		}

		$cap     = 'term' === $object_type ? 'edit_term' : 'edit_post';
		$allowed = false;

		ERankly_ML_Sites::switch_to_blog_for_link( $blog_id );

		try {
			$allowed = current_user_can( $cap, $object_id );
		} finally {
			ERankly_ML_Sites::restore_blog_for_link();
		}
	...
	/**
	 * Whether a specific object is a member of the given translation group.
	 *
	 * Used to confirm an unlink request targets a relation that genuinely
	 * belongs to the source object's group before deleting anything.
	 *
	 ...
	 * @return bool
	 */
	private function group_contains_object( int $group_id, int $blog_id, string $object_type, int $object_id ): bool {
		foreach ( $this->repo->get_group_members( $group_id ) as $member ) {
			if ( (int) $member['blog_id'] === $blog_id
				&& (string) $member['object_type'] === $object_type
				&& (int) $member['object_id'] === $object_id ) {
				return true;
			}
		}
	...
	public function save_ml_sites(): void {
		...
		$redirect = network_admin_url( 'settings.php?page=erankly&erankly_tab=multilingual' );
		...
		foreach ( $this->get_translation_target_blog_ids( $source_blog_id ) as $blog_id ) {
			...
			if ( $object_id > 0 ) {
				...
				if ( $linked_post instanceof WP_Post ) {
					$title = $this->decode_admin_label( get_the_title( $linked_post ) );
				...
				if ( $object_id > 0 ) {
					...
					if ( 'term' === $object_type ) {
						...
						if ( $t instanceof WP_Term ) {
							$linked_title = $this->decode_admin_label( $t->name );
						...
						if ( $p instanceof WP_Post ) {
							$linked_title = $this->decode_admin_label( get_the_title( $p ) );
			...
			<div class="notice notice-info inline erankly-ml-empty">
				...
				<p class="description">
					...
					if ( current_user_can( 'manage_network_options' ) ) {
						...
						esc_html_e( 'Ask a network administrator to enable sites in EasyRankly, then come back here to link translations.', 'easyrankly' );
					}
					?>
				</p>
			</div>
	...
	public function render_network_panel(): void {
		...
		<div class="erankly-settings-fields erankly-ml-network">
					...
					foreach ( $sites as $site ) :
						...
						<tr>
							...
							<td class="erankly-ml-col-code">
								...
								<span class="erankly-ml-derived" title="<?php esc_attr_e( "Detected from the site's language setting (used if left blank).", 'easyrankly' ); ?>">
							...
							<td class="erankly-ml-col-toggle">
								<label class="erankly-ml-toggle">
									<input type="checkbox" class="erankly-toggle" name="<?php echo esc_attr( $field ); ?>[enabled]" value="1" <?php checked( $enabled ); ?>>
			...
			<div class="erankly-settings-section">
				<h3 class="erankly-section-title"><?php esc_html_e( 'Frontend Shortcodes', 'easyrankly' ); ?></h3>
				<div class="erankly-ml-shortcodes-docs erankly-card">
					<p class="description">
