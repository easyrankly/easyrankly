...
function erankly_render_post_visibility_fields( WP_Post $post ): void {
	...
	<fieldset class="erankly-field erankly-checkboxes">
		...
		<?php else : ?>
			<label><input type="checkbox" class="erankly-toggle" name="erankly_noindex" value="1" <?php checked( $noindex ); ?>> <?php esc_html_e( 'Noindex', 'easyrankly' ); ?></label><br>
			<label><input type="checkbox" class="erankly-toggle" name="erankly_nofollow" value="1" <?php checked( $nofollow ); ?>> <?php esc_html_e( 'Nofollow', 'easyrankly' ); ?></label><br>
			<label><input type="checkbox" class="erankly-toggle" name="erankly_noarchive" value="1" <?php checked( $noarchive ); ?>> <?php esc_html_e( 'Noarchive', 'easyrankly' ); ?></label><br>
 ...
 *
 * @param array<string,string> $examples Example values (e.g. the first
 *                                        post's title for {{post_title}})
 *                                        used to resolve this field's
 *                                        "friendly" preview. A token missing
 *                                        from this map is left literal.
 * @return void
 */
...
function erankly_render_schema_targeting_fields( array $block, string $index, string $name_prefix, bool $enabled ): void {
	...
	<fieldset class="erankly-schema-targeting">
		...
		<label>
			<input type="checkbox" class="erankly-toggle" name="<?php echo esc_attr( $name_prefix ); ?>[<?php echo esc_attr( $index ); ?>][enabled]" value="1" <?php checked( $enabled ); ?>>
		...
		<div class="erankly-schema-targeting-grid">
			<div>
				...
				<?php foreach ( $contexts as $context => $label ) : ?>
					<label>
						<input type="checkbox" class="erankly-toggle" name="<?php echo esc_attr( $name_prefix ); ?>[<?php echo esc_attr( $index ); ?>][target_contexts][]" value="<?php echo esc_attr( $context ); ?>" <?php checked( in_array( $context, $target_contexts, true ) ); ?>>
			...
			<div>
				...
				<?php foreach ( erankly_get_public_post_types() as $post_type => $object ) : ?>
					<label>
						<input type="checkbox" class="erankly-toggle" name="<?php echo esc_attr( $name_prefix ); ?>[<?php echo esc_attr( $index ); ?>][target_post_types][]" value="<?php echo esc_attr( $post_type ); ?>" <?php checked( in_array( $post_type, $target_post_types, true ) ); ?>>
